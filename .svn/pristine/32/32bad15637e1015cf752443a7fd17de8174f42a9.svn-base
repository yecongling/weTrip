...
import com.foo.wetrip.util.AlipayConfig;
...
public class AlipayController {
    ...
    public String goAlipay(String orderNum, HttpServletRequest request, HttpServletRequest response) throws Exception {
        ...
        List<Oders> ordersList = orderService.getOrdersList(orderNum);
        Oders order = ordersList.get(0);
        Scenic scenic = ScenicService.getScenicById(order.getScenicId());
        ...
        //付款金额，必填
        String total_amount = order.getPayPrice();
        //订单名称，必填
        String subject = scenic.getScenicName();
        //商品描述，可空
    ...
    public ModelAndView alipayReturnNotice(HttpServletRequest request, HttpServletRequest response) throws Exception {
        ...
        if (signVerified) {
            ...
            List<Oders> ordersList = orderService.getOrdersList(out_trade_no);
            Oders order = ordersList.get(0);
            Scenic scenic = ScenicService.getScenicById(order.getScenicId());
            ...
            // log.info("* 购买产品: {}", scenic.getScenicName());
            // log.info("***************************************************************");
            mv.addObject("out_trade_no", out_trade_no);
            mv.addObject("trade_no", trade_no);
            mv.addObject("total_amount", total_amount);
            mv.addObject("scenicName", scenic.getScenicName());
